...
const { getUser, getUserById, addUser, updateCredentials, resetUsers } = require('./db');
...
const RP_CLIENT_IDS = [ RP_CLIENT_ID, RP_MULTI_IDP_CLIENT_ID, 'https://identity-demos.dev' ];
...
router.post('/account', csrfCheck, apiSessionCheck, (req, res) => {
  ...
  if (user.username !== 'demo@example.com' && (
      ...
      !/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/.test(req.body.picture))) {
...
router.get('/metadata', (req, res) => {
  ...
  return res.json({
    privacy_policy_url: `${ORIGIN}/privacy_policy`,
    terms_of_service_url: `${ORIGIN}/terms_of_service`,
  })
});
...
const isValidOrigin = (originStr) => {
  ...
  return (
    new URL(RP_ORIGIN).toString().includes(originStr) ||
    new URL(RP_MULTI_IDP_ORIGIN).toString().includes(originStr) ||
    originStr === new URL("http://localhost:8080").toString() ||
    originStr ===
      new URL("https://project-sesame.googleplex.com/").toString() ||
    originStr ===
      new URL("https://project-sesame-426206.appspot.com/").toString() ||
    originStr ===
      new URL("https://dev-dot-project-sesame.googleplex.com/").toString() ||
    originStr ===
      new URL("https://luminous-accurate-parenthesis.glitch.me/").toString()
  );
};
...
router.post('/idtokens', csrfCheck, apiSessionCheck, (req, res) => {
  ...
  const { client_id, nonce, account_id, consent_acquired, disclosure_text_shown, param_scope } = req.body;
  let user = res.locals.user;
  ...
  const currentOrigin = new URL(req.headers.origin).toString().replace(/\/$/, "");
  ...
  if (
    ...
    // TODO: This is adding more than one client IDs to the array. Fix it.
  ...
  if (user.status === '') {
    ...
    if (param_scope) {
      ...
      return res.json({
        continue_on: `/authorization?client_id=${client_id}&scope=${param_scope}&nonce=${nonce}`
      });
    }
